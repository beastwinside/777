...
var searcharr=['4g内存','8g内存','16g内存'];


function dealram(searchname){
...
let dianpu=dian;
let jg=jiage;
	let xhflag=xh.match(/台式机内存|笔记本内存|工作站内存/g);  //分理出内存类型对应的位置。
	let xhcon=xh.split(/台式机内存|笔记本内存|工作站内存/g);  //根据内存类型，分割字符串
